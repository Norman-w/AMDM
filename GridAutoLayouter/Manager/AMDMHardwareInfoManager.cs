...
namespace GridAutoLayouter
{
    ...
    public class AMDMHardwareInfoManager : AMDMStockLoader
    {
        ...
        /// <returns></returns>
        public AMDM_Stock CreateAndJoinStock(ref AMDM_Machine machine,
        ...
        {
            AMDM_Stock newStock = CreateStock(machine.Id, CenterDistanceBetweenTwoGrabbers,IndexOfMachine,MaxFloorsHeightMM,MaxFloorWidthMM, SerialNumber,XOffsetFromStartPointMM,YOffsetFromStartPointMM);
        ...
        public AMDM_Stock CreateStock(int machineId,
            float CenterDistanceBetweenTwoGrabbers,
            int IndexOfMachine,
            int MaxFloorsHeightMM,
            int MaxFloorWidthMM,
        ...
        public bool JoinStock(AMDM_Machine machineRef, AMDM_Stock stock)
        {
            if (client.AddStock(stock))
            {
                if (machineRef != null)
                {
                    if (machineRef.Stocks == null)
                    {
                        machineRef.Stocks = new List<AMDM_Stock>();
                    }
                    machineRef.Stocks.Add(stock);
                }
                return true;
            }
            else
            ...
            for (int i = 0; i < upPartFloorCount; i++)
            {
                ...
                else
                {
                    for (int j = 0; j < gridCountPerUpPartFloor; j++)
                    {
                        ...
                        float left = width * j;
                        AMDM_Grid newGrid = this.CraeteAndJoinNewGrid(newFloor, null, left, width, gridIndexOfStock++);
                    }
                }
            }
            #endregion
            ...
            for (int i = 0; i < downPartFloorCount; i++)
            {
                ...
                else
                {
                    for (int j = 0; j < gridCountPerDownPartFloor; j++)
                    {
                        ...
                        float left = width * j;
                        AMDM_Grid newGrid = this.CraeteAndJoinNewGrid(newFloor, null, left, width, gridIndexOfStock + j);
                    }
                }
            }
            #endregion
        ...
        /// <returns></returns>
        public AMDM_Grid CraeteAndJoinNewGrid(AMDM_Floor floor, Nullable<int> indexOfFloor, float leftMM, float widthMM, int indexOfStock)
        {
            ...
            newGrid.IndexOfStock = indexOfStock;
